...
let lastKitJson: unknown | null = null;
app.post('/api/brandkit', (req: Request, res: Response) => {
  lastKitJson = req.body;
  res.json({ ok: true });
});
app.get('/api/brandkit', (_req: Request, res: Response) => {
  if (!lastKitJson) return res.status(404).json({ error: 'not found' });
...
app.get('/api/hints', async (req: Request, res: Response) => {
  ...
  if (MOCK) return res.json({ logo: sampleLogoDataUrl, themeColor: '#0A84FF', fonts: ['Inter', 'Georgia'] });
  try {
...
app.post('/api/analyze/colors', async (req: Request, res: Response) => {
  if (MOCK) return res.json({ swatches: mockSwatches });
  // Adapter example: Imagga
  const { imageUrl } = z.object({ imageUrl: z.string().url().optional() }).parse(req.body);
  if (!imageUrl) return res.json({ swatches: mockSwatches });
  try {
    ...
    const swatches = (data?.result?.colors?.image_colors ?? [])
      .slice(0, 8)
      .map((c: any) => ({ name: c.closest_palette_color ?? 'Color', hex: `#${String(c.html_code).replace('#','')}`, role: 'accent' }));
    res.json({ swatches });
  } catch (e) {
...
app.post('/api/analyze/fonts', async (_req: Request, res: Response) => {
  if (MOCK) return res.json({ primary: { family: 'Inter', source: 'google-fonts' }, options: [{ family: 'Roboto' }, { family: 'Open Sans' }] });
  ...
  try {
    ...
    res.json({ primary: { family: fam, source: 'google-fonts' }, options: (data.items ?? []).slice(0, 5).map((it: any) => ({ family: it.family, source: 'google-fonts' })) });
  } catch (e) {
    res.json({ primary: { family: 'Roboto', source: 'google-fonts' }, options: [{ family: 'Inter' }, { family: 'Open Sans' }] });
  }
});

app.post('/api/analyze/tone', async (_req: Request, res: Response) => {
  if (MOCK) return res.json({ tone: 'friendly' });
  res.json({ tone: 'minimal' });
});

app.get('/api/fonts/google', async (_req: Request, res: Response) => {
  try {
    const key = process.env.GOOGLE_FONTS_KEY;
    const r = await fetch(`https://www.googleapis.com/webfonts/v1/webfonts?sort=popularity&key=${key}`);
...
const sampleLogoDataUrl =
  'data:image/svg+xml;base64,' + Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256"><rect width="256" height="256" fill="#0A84FF"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-size="64" fill="#fff" font-family="Arial, sans-serif">SBA</text></svg>`).toString('base64');
